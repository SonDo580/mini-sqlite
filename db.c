...
const u_int32_t ROWS_PER_PAGE = PAGE_SIZE / ROW_SIZE; // rows should not cross page boundary
...
void *get_page(Pager *pager, u_int32_t page_num)
{
    ...
    if (pager->pages[page_num] == NULL)
    {
        ...
        {
            num_pages += 1; // partial page at end of file
        }
...
void db_close(Table *table)
{
    ...
    // (this should not be needed after switching to B-tree)
    u_int32_t num_additional_rows = table->num_rows % ROWS_PER_PAGE;
    if (num_additional_rows > 0)
    {
        u_int32_t last_page_num = num_full_pages;
        if (pager->pages[last_page_num] != NULL)
        {
            pager_flush(pager, last_page_num, num_additional_rows * ROW_SIZE);
