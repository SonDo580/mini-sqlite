...
typedef struct
{
    char *buffer;         // buffer containing the read line
...
{
    u_int32_t id;
    char username[COLUMN_USERNAME_SIZE + 1]; // plus 1 for '\0'
    char email[COLUMN_EMAIL_SIZE + 1];       // plus 1 for '\0'
} Row;
...
const u_int32_t PAGE_SIZE = 4096; // 4KiB, matches OS virtual memory page size
...
/* Represent a location in the table. */
typedef struct
{
    Table *table;
    u_int32_t page_num;
    u_int32_t cell_num;
    bool end_of_table; // Indicate the position one past the last element
} Cursor;
 ...
 * - node type
 * - is this the root?
 * - pointer to parent (allow finding sibling)
 */
 ...
 * - ... common headers
 * - number of cells (a cell is a key-value pair)
 */
...
const u_int32_t LEAF_NODE_SPACE_FOR_CELLS =
    PAGE_SIZE - LEAF_NODE_HEADER_SIZE; // 1 node <=> 1 page
const u_int32_t LEAF_NODE_MAX_CELLS = LEAF_NODE_SPACE_FOR_CELLS / LEAF_NODE_CELL_SIZE;

NodeType get_node_type(void *node)
{
    u_int8_t type = *((u_int8_t *)(node + NODE_TYPE_OFFSET));
...
void serialize_row(Row *source, void *destination)
{
    ...
    // Use `strncpy` instead of `memcpy` to ensure all bytes are initialized
...
void *get_page(Pager *pager, u_int32_t page_num)
{
    ...
    {
        printf("Tried to fetch page number out of bounds. %d + 1 > %d\n", page_num, TABLE_MAX_PAGES);
        exit(EXIT_FAILURE);
    }

    if (pager->pages[page_num] == NULL)
    {
        // Cache miss. Allocate memory (still empty page).
 ...
 * - the position of another key that need to move to insert the new key.
...
Cursor *leaf_node_find(Table *table, u_int32_t page_num, u_int32_t key)
{
    ...
    // Binary search
    u_int32_t left = 0;
    u_int32_t right = num_cells; // ([!] avoid underflow of `right`)

    while (left < right)
    {
        u_int32_t mid = (left + right) / 2;
        u_int32_t key_at_mid = *leaf_node_key(node, mid);
...
void db_close(Table *table)
{
    ...
    // Close DB file.
    // Free memory for pager and table.
...
void read_input(InputBuffer *input_buffer)
{
    ...
    input_buffer->buffer[bytes_read - 1] = 0; // null terminator '\0'
}
...
void leaf_node_insert(Cursor *cursor, u_int32_t key, Row *value)
{
    ...
    {
        // Shift right to make room for new cell
        // ([!] avoid underflow of i)
